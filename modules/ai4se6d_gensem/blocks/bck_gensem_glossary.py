...
_ENTRIES = [
    ...
    ("Decision Tier", "Classification of a decision by reversibility and impact — Auto (agent decides, logs), Inform (agent decides, explains in one line), Gate (full analysis, human validates). Calibrated by the HUG profile (P7)"),
    ...
    ("HUG", "Human User Grounding \u2014 /gse:hug captures developer profile across 13 dimensions"),
    ...
    ("Mode", "Ceremony level \u2014 Micro / Lightweight / Full \u2014 recommended by the complexity assessment (7 structural signals + triviality pre-filter, first match wins)"),
    ...
    ("Spike", "Complexity-boxed throwaway exploration (max 3 points) \u2014 bypasses REQS/TESTS guardrails, must produce a DEC- artifact; only the decision survives"),
    ("Sprint", "Complexity-boxed iteration \u2014 ends when the point budget is consumed or all tasks are delivered, not by calendar"),
