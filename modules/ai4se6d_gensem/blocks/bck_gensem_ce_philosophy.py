...
def build():
    with st_block(s.project.containers.page_fill_top):
        ...
        with st_block(s.project.containers.callout):
            st_write(
                bs.callout_text,
                "Foundational principle: each unit of engineering work should make "
                "subsequent units easier \u2014 not harder.",
            )
        ...
        with st_grid(
            ...
            with g.cell():
                with st_block(s.project.containers.cell_accent_bg + s.project.containers.cell_pad_md):
                    ...
                    with st_list(li_style=bs.body, list_type=lt.unordered) as l:
                        with l.item():
                            st_write(bs.body, (bs.keyword, "5 independent phases"), ", composable not sequential")
                        with l.item():
                            st_write(bs.body, "Each phase produces a concrete artifact")
                        with l.item():
                            st_write(bs.body, "Phases can be run in any order or skipped")
                        with l.item():
                            st_write(bs.body, "Artifacts accumulate and feed future cycles")
