                ...
                with g.cell():
                    st_hover_tooltip(
                        ...
                        entries=[
                            ...
                            ("Installation", "curl <repo>/install.sh | sh — run inside the project; the single agent on PATH is auto-picked (several agents: set GSE_PLATFORM). Or python3 install.py --platform claude|cursor|opencode|codex|gemini|both|all --mode project|machine|sandbox."),
                            ("Portability", "A project install travels with its folder: opened on another machine (or after a clone), no reinstall — accept the workspace trust dialog and run /reload-plugins; the deterministic guardrails (carried by .claude/settings.json) are active even before that."),
