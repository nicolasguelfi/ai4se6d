"""Trainer profile — Nicolas Guelfi (imported from HTML)."""
# @guideline: maximize-viewport
from streamtex import *
from streamtex.styles import Style as ns
from streamtex.enums import Tags as t
from custom.styles import Styles as s


# Viewport-filling container
_page_fill = ns(
    "display:flex;flex-direction:column;justify-content:flex-start;"
    "min-height:85vh;gap:1.5rem;",
    "trainer_ng_page_fill",
)

# Shared image path
_IMG = "_SHARED/bck_trainer_ng"

# Cell style for the profile grid (table-roadmap pattern)
_cell = Style.create(
    ns("background-color: rgba(122, 184, 245, 0.08); "
       "border: 1px solid rgba(122, 184, 245, 0.3); "
       "border-radius: 10px; padding: 12px 16px;", "trainer_cell_bg")
    + s.container.layouts.vertical_center_layout
    + s.center_txt,
    "trainer_cell",
)


class BlockStyles:
    """Trainer profile — Nicolas Guelfi."""
    heading = s.project.titles.section_title + s.center_txt
    name = Style.create(
        s.project.colors.primary + s.bold + s.huge + s.center_txt,
        "trainer_ng_name",
    )
    role = Style.create(
        s.project.titles.body + s.center_txt,
        "trainer_ng_role",
    )
    category_title = Style.create(
        s.project.colors.highlight + s.bold + s.Large + s.text.wrap.hyphens,
        "trainer_ng_cat_title",
    )
    category_item = Style.create(
        s.Large + s.text.wrap.hyphens,
        "trainer_ng_cat_item",
    )
    link_label = Style.create(
        s.project.colors.accent + s.Large + s.center_txt,
        "trainer_ng_link",
    )
bs = BlockStyles


def build():
    # ── Slide 1: Title + Photo + Name ──
    with st_block(_page_fill):
        with st_block(s.center_txt):
            st_write(bs.heading, "Who?", tag=t.div, toc_lvl="1")

            st_image(s.none, uri=f"{_IMG}/image5.png", width="200px", height="200px")

            st_write(bs.name, "Nicolas Guelfi",
                     link="https://drive.google.com/file/d/1FaGAxN4-zK2YUBeXsYJsxj3BWA3eJUas/view")
            st_write(bs.role, "University of Luxembourg  ·  Right-On-Skill")

    st_slide_break()

    # ── Slide 2: Profile grid (2x2) ──
    with st_block(_page_fill):
        with st_block(s.center_txt):
            st_write(bs.heading, "Profile", tag=t.div, toc_lvl="2")

            # Row 1: Activities | Software Engineering
            with st_grid(
                cols="repeat(auto-fit, minmax(350px, 1fr))",
                gap="12px",
                cell_styles=_cell,
            ) as g:
                with g.cell():
                    st_write(bs.category_title, "Activities")
                    st_space("v", 0.5)
                    st_write(bs.category_item, "Education / Training")
                    st_write(bs.category_item, "Research / Development")
                with g.cell():
                    st_write(bs.category_title, "Software Engineering")
                    st_space("v", 0.5)
                    st_write(bs.category_item, "Requirements engineering")
                    st_write(bs.category_item, "Critical systems")

            st_space("v", 0.5)

            # Row 2: Contexts | Artificial Intelligence
            with st_grid(
                cols="repeat(auto-fit, minmax(350px, 1fr))",
                gap="12px",
                cell_styles=_cell,
            ) as g:
                with g.cell():
                    st_write(bs.category_title, "Contexts")
                    st_space("v", 0.5)
                    st_write(bs.category_item, "University of Luxembourg")
                    # Institution logos
                    with st_grid(cols="repeat(auto-fit, minmax(60px, 1fr))", gap="8px") as lg:
                        with lg.cell():
                            st_image(s.none, uri=f"{_IMG}/image7.png", width="5vw")
                        with lg.cell():
                            st_image(s.none, uri=f"{_IMG}/image8.png", width="5vw")
                        with lg.cell():
                            st_image(s.none, uri=f"{_IMG}/image3.png", width="5vw")
                        with lg.cell():
                            st_image(s.none, uri=f"{_IMG}/image9.png", width="5vw")
                    st_space("v", 0.5)
                    st_write(bs.category_item, "Right-On-Skill sarl")
                    with st_grid(cols="repeat(auto-fit, minmax(60px, 1fr))", gap="8px") as lg:
                        with lg.cell():
                            st_image(s.none, uri=f"{_IMG}/image6.png", width="5vw")
                        with lg.cell():
                            st_image(s.none, uri=f"{_IMG}/image2.png", width="5vw")
                        with lg.cell():
                            st_image(s.none, uri=f"{_IMG}/image4.png", width="5vw")
                with g.cell():
                    st_write(bs.category_title, "Artificial Intelligence")
                    st_space("v", 0.5)
                    st_write(bs.category_item, "Generative AI")
                    st_write(bs.category_item, "Deep Learning")
                    st_write(bs.category_item, "Expert systems")

    st_slide_break()

    # ── Slide 3: More information + LinkedIn QR ──
    with st_block(_page_fill):
        with st_block(s.center_txt):
            st_write(bs.heading, "More Information", tag=t.div)
            st_image(s.none, uri=f"{_IMG}/image1.png", width="200px")
            st_write(bs.link_label, "Connect on LinkedIn")
