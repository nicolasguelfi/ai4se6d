"""Slide — Step 2: Embeddings: balanced image + key points."""
...
class BlockStyles:
    """Slide: Embeddings — maximize-viewport archetype: balanced."""
...
_PROMPT = (
    f"{_PREFIX} Abstract 3D coordinate space with scattered glowing dots. Similar concepts "
    "clustered together in electric blue, distant concepts in teal, with thin white connection "
    "lines between neighbors. One amber dot highlighted. Represents semantic vector space. "
    f"{_SUFFIX}"
)
...
def build():
    with st_block(_page_fill):
        with st_block(s.center_txt):
            st_write(bs.heading, "Step 2: Embeddings", tag=t.div, toc_lvl="1")
                    ...
                    with st_list(
                        ...
                        with l.item():
                            st_write(
                                bs.body,
                                (bs.keyword, "Vectors"),
                                (bs.body, " \u2014 Each token becomes a 4,096-12,288 dimension vector"),
                            )
                        with l.item():
                            st_write(
                                bs.body,
                                (bs.keyword, "Semantics"),
                                (bs.body, " \u2014 Similar meanings = close in vector space"),
                            )
                        with l.item():
                            st_write(
                                bs.body,
                                (bs.keyword, "Learned"),
                                (bs.body, " \u2014 Relationships discovered from billions of examples"),
                            )
